...
차트 아래에는 **주차장 배치 구조도**가 있다. 지도가 아니라 구조도라는 점이 중요하다 —
방위와 거리는 나타내지 않는다. 터미널 소속, 도보로 갈 수 있는지 셔틀을 타야 하는지, 같은
부지에 평면 주차장과 주차타워가 함께 있는지처럼 **확인된 관계만** 담았다. T1의 P1/P2가
동편인지 서편인지는 출처마다 서술이 달라 확인되지 않았고, 위치가 틀린 배치도는 없느니만
못하므로 주장하지 않는다.

여기서도 구역을 누르면 표·차트와 같은 선택 상태로 연동된다.

두 가지는 알아둘 만하다. 공개 데이터에 P3 다음이 P5인 것은 **P4가 상주직원 전용**이라
집계에서 빠지기 때문이다. 그리고 **T1의 P1·P2와 T2의 P1·P2는 완전히 다른 부지**다 —
내비게이션에 번호만 넣으면 엉뚱한 터미널로 간다.
